...
const OPTIONS: {
...
}[] = [
  {
    ...
    emoji: "😤",
    label: "tense",
    desc: "holding something, can't quite settle",
  },
  {
    value: "neutral",
    emoji: "😐",
    label: "neutral",
    desc: "not much either way, just here",
  },
  {
    value: "relaxed",
    emoji: "😌",
    label: "relaxed",
    desc: "soft, open, breathing easy",
  },
];

const BodyStep = ({ onNext }: StepProps) => {
  const [selected, setSelected] = useState<BodyState | null>(null);
  const setNervousSystemState = useAnchorStore(
    (s) => s.setNervousSystemState
  );

  const handleSelect = (value: BodyState) => {
    setSelected(value);
    setNervousSystemState("body", value);
